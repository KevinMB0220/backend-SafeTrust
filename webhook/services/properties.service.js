...
function generateSearchMockData(filters) {
  ...
  if (filters.minPrice !== undefined) {
    filtered = filtered.filter(i => i.price >= filters.minPrice);
  }
  if (filters.maxPrice !== undefined) {
    filtered = filtered.filter(i => i.price <= filters.maxPrice);
  }
...
function generateMockData(type, limit, offset) {
  ...
  const filtered = type === 'all' ? mockData : mockData.filter(i => i.type === type);
